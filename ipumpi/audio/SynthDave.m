...
- (void)emptyQueue  
{
    ...
    LOOPIT(queuePtr)
    {
        ...
        portamentoTime    = noteQueue[7][loop];
        portamentoLastNote = noteQueue[8][loop];
...
- (void)queueNote:(int)midiNote :(int)wnum :(int)type
{
    ...
    if (queuePtr < MAX_QUEUE-1)
    {
        ...
        noteQueue[7][queuePtr] = portamentoTime;
        noteQueue[8][queuePtr] = portamentoLastNote;
...
//            }
			...
			else if (tones[n].toneType == SAMPLE_VOICE) //samples ! but not perc!
			{
                ...
                if (tones[n].phase  < 0.0)
                    {
                        tones[n].phase = 0.0;
                        NSLog(@" ERROR!! negative phase on sample voice %d",n);
                    }
			...
			{   
                ...
                {
                    tones[n].phase = 0.0;
                    NSLog(@" ERROR!! negative phase on perc voice %d",n);
                }
...
{ 	 
    ...
    NSLog(@" in dumpAudio...write output to %@",recFileFullPath);
...
-(void)startRecording:(int)newlen
{
    ...
    recsize = newlen * DEFAULT_SAMPLE_RATE * 2 * sizeof(short);
